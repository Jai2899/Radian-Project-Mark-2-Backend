...
module.exports = {
  ...
  createEvent: async (args, req) => {
    if (!req.isAuth) {
      ...
      const originalTitleQuote = Math.ceil(
        +args.eventInput.purchaseAmount / 126
      );
      ...
      const BDST = 0;
      const BMST =
        (+args.eventInput.purchaseAmount - +args.eventInput.loanAmount) *
        0.0105;
      const BMIT =
      ...
      const event = new Event({
        ...
        TDRF: TDRF,
        TTTOGF: TTTOGF,
        creator: "6192de84ffddf7a64701791b",
      });
      let createdEvent;
      try {
        const result = await event.save();
        createdEvent = transformEvent(result);
        const creator = await User.findById("6192de84ffddf7a64701791b");
